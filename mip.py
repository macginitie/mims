...
def is_high_greyscale_contrast( px1, px2, paramlist ):
    threshold = paramlist[0] if len(paramlist) > 0 else 10
    return greyscale_contrast( px1, px2 ) > threshold
    

def is_high_contrast( px1, px2, paramlist ):
    threshold = paramlist[0] if len(paramlist) > 0 else 10
    return euclidean_dist( px1, px2 ) > (threshold * threshold)
...
def nbr_is_black( pixel, neighbors, threshold, dot_size ):
    ...
    count = 0
    for nbrpxl in neighbors:
        if sum_channels( nbrpxl ) < threshold:
            count += 1
    if count > 0:
...
def maybe_black_dot(pixel, neighborhood, paramlist = []):
    greyval = sum_channels( pixel )
    threshold = 200 if len(paramlist) < 1 else paramlist[0]
    dot_size = 2 if len(paramlist) < 2 else paramlist[1]
    if nbr_is_black( pixel, neighborhood, threshold, dot_size ):
        return black
    else:
    	return white
...
def darken( pixel, x, y, paramlist = [] ):
    ...
    return (max(0, pixel[0] - darkness_threshold), max(0, pixel[1] - darkness_threshold), max(0, pixel[2] - darkness_threshold) )
...
def blacken_below_threshold( pixel, x, y, paramlist = [] ):
    threshold = 700 if len(paramlist) == 0 else paramlist[0]
    if sum_channels( pixel ) > threshold:
        return pixel
    return black
    

def black_or_white( pixel, x, y, paramlist = [] ):
    threshold = 386 if len(paramlist) == 0 else paramlist[0]
    if is_above_darkness_threshold( pixel, threshold ):
        return white
    return black
...
def replace_black_with_random( pixel, x, y, paramlist = [] ):
    threshold = 386 if len(paramlist) == 0 else paramlist[0]
    if not is_above_darkness_threshold( pixel, threshold ):
        return random_color()
    return pixel
...
def stipple( pixel, x, y, paramlist = [] ):
    threshold = paramlist[0] if len(paramlist) > 0 else 200
...
def enhanced_stipple( pixel, x, y, paramlist = [] ):
    ...
    if brightness > leave_white: return white
    if brightness < leave_black: return black
